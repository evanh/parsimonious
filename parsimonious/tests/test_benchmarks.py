...
class BenchmarkTests(unittest.TestCase):
    def test_lists_vs_dicts(self):
        ...
        assert list_time < dict_time, '%s < %s' % (list_time, dict_time)
    ...
    def test_call_vs_inline(self):
        ...
        assert no_call < call, '%s (no call) < %s (call)' % (no_call, call)
    ...
    def test_startswith_vs_regex(self):
        ...
        assert startswith_time < re_time, '%s (startswith) < %s (re)' % (startswith_time, re_time)
