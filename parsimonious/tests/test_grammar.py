...
class BootstrappingGrammarTests(TestCase):
    ...
    def test_quantifier(self):
        ...
        assert quantifier.parse(text) == Node(quantifier, text, 0, 1, children=[
    ...
    def test_spaceless_literal(self):
        ...
        assert spaceless_literal.parse(text) == Node(spaceless_literal, text, 0, len(text), children=[
                Node(spaceless_literal.members[0], text, 0, len(text))])
        text = r'''r"\""'''
        assert spaceless_literal.parse(text) == Node(spaceless_literal, text, 0, 5, children=[
                Node(spaceless_literal.members[0], text, 0, 5)])

    def test_regex(self):
        text = '~"[a-zA-Z_][a-zA-Z_0-9]*"LI'
        regex = rule_grammar['regex']
        assert rule_grammar['regex'].parse(text) == Node(regex, text, 0, len(text), children=[
    ...
    def test_successes(self):
        ...
        assert rule_grammar['label'].parse('_')
        assert rule_grammar['label'].parse('jeff')
        assert rule_grammar['label'].parse('_THIS_THING')

        assert rule_grammar['atom'].parse('some_label')
        assert rule_grammar['atom'].parse('"some literal"')
        assert rule_grammar['atom'].parse('~"some regex"i')

        assert rule_grammar['quantified'].parse('~"some regex"i*')
        assert rule_grammar['quantified'].parse('thing+')
        assert rule_grammar['quantified'].parse('"hi"?')

        assert rule_grammar['term'].parse('this')
        assert rule_grammar['term'].parse('that+')

        assert rule_grammar['sequence'].parse('this that? other')

        assert rule_grammar['ored'].parse('this / that+ / "other"')

        # + is higher precedence than &, so 'anded' should match the whole
        # thing:
        assert rule_grammar['lookahead_term'].parse('&this+')

        assert rule_grammar['expression'].parse('this')
        assert rule_grammar['expression'].parse('this? that other*')
        assert rule_grammar['expression'].parse('&this / that+ / "other"')
        assert rule_grammar['expression'].parse('this / that? / "other"+')
        assert rule_grammar['expression'].parse('this? that other*')

        assert rule_grammar['rule'].parse('this = that\r')
        assert rule_grammar['rule'].parse('this = the? that other* \t\r')
        assert rule_grammar['rule'].parse('the=~"hi*"\n')

        assert rule_grammar.parse('''
            this = the? that other*
            that = "thing"
            the=~"hi*"
            other = "ahoy hoy"
...
class RuleVisitorTests(TestCase):
    ...
    def test_round_trip(self):
        ...
        text = '98'
        assert default_rule.parse(text) == Node(default_rule, text, 0, 2)
    ...
    def test_optional(self):
        ...
        # Literal within.
        assert default_rule.parse(howdy) == Node(default_rule, howdy, 0, 5, children=[
...
class GrammarTests(TestCase):
    ...
    def test_expressions_from_rules(self):
        ...
        assert tree == Node(greeting_grammar['greeting'], 'hi', 0, 2, children=[
    ...
    def test_unicode(self):
        ...
        assert lines[0] == 'bold_text = bold_open text bold_close'
        assert 'text = ~"[A-Z 0-9]*"i%s' % ('u' if sys.version_info >= (3,) else '') in lines
        assert 'bold_open = "(("' in lines
        assert 'bold_close = "))"' in lines
        assert len(lines) == 4
    ...
    def test_match(self):
        ...
        grammar = Grammar(r"""
                          ...
                          """)
        s = ' ((boo))yah'
        assert grammar.match(s, pos=1) == Node(grammar['bold_text'], s, 1, 8, children=[
    ...
    def test_comments(self):
        ...
        grammar = Grammar(r"""# This is a grammar.
                          ...
                          #Oh yeah.#""")  # Make sure a comment doesn't need a
                                          # \n or \r to end.
        assert list(sorted(str(grammar).splitlines())) == ['''bold_text = stars text stars''',
             # TODO: Unicode flag is on by default in Python 3. I wonder if we
             # should turn it on all the time in Parsimonious.
             '''stars = "**"''',
             '''text = ~"[A-Z 0-9]*"i%s''' % ('u' if sys.version_info >= (3,)
                                              else '')]
    ...
    def test_multi_line(self):
        ...
        grammar = Grammar("""
            ...
            """)
        assert grammar.parse('((booyah))') is not None
    ...
    def test_not(self):
        ...
        assert grammar.parse('argle') is not None
    ...
    def test_lookahead(self):
        ...
        s = 'arp'
        assert grammar.parse('arp') == Node(grammar['starts_with_a'], s, 0, 3, children=[
    ...
    def test_parens(self):
        ...
        s = 'chitty bang bang'
        assert str(grammar.parse(s)) == """<Node called "sequence" matching "chitty bang bang">
    ...
    def test_right_recursive(self):
        ...
        grammar = Grammar("""
            ...
            digit = ~r"[0-9]"
            """)
        assert grammar.parse('12') is not None

    @pytest.mark.skip(reason='We have yet to make the grammar compiler detect these.')
    ...
    def test_simple_custom_rules(self):
        ...
        s = '[6]'
        assert grammar.parse(s) == Node(grammar['bracketed_digit'], s, 0, 3, children=[
    ...
    def test_lazy_custom_rules(self):
        ...
        s = '45'
        assert grammar.parse(s) == Node(grammar['forty_five'], s, 0, 2, children=[
    ...
    def test_unconnected_custom_rules(self):
        ...
        s = '4'
        assert grammar.parse(s) == Node(grammar['one_char'], s, 0, 1)

    def test_callability_of_routines(self):
        assert is_callable(function_rule)
        assert is_callable(self.method_rule)
        assert is_callable(self.rules['descriptor_rule'])
    ...
    def test_callability_custom_rules(self):
        ...
        assert rule_names == ['function', 'method', 'descriptor']
    ...
    def test_lazy_default_rule(self):
        ...
        grammar = Grammar(r"""
            ...
            text        = "hi"
            """)
        assert grammar.parse('hi') == Node(grammar['text'], 'hi', 0, 2)
...
class TokenGrammarTests(TestCase):
    ...
    def test_parse_success(self):
        ...
        grammar = TokenGrammar("""
            ...
            token1 = "token1"
            """)
        assert grammar.parse(s) == Node(grammar['foo'], s, 0, 2, children=[
